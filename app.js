...
function reportsPdfCover(doc,payload,survey,client){
  ...
  doc.setFillColor(37,99,235);doc.roundedRect(M,25,70,11,5.5,5.5,'F');
  doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(14);doc.text('PesquisaPro',M+35,32.5,{align:'center'});
  doc.setFont('helvetica','bold');doc.setFontSize(27);const titleLines=doc.splitTextToSize(payload.title||'Relatório de resultados',bodyW);const titleY=76;doc.text(titleLines.slice(0,4),M,titleY,{lineHeightFactor:1.08});
...
async function reportsCreateMultiCrossPdfBlob(payload){
  ...
  reportsPdfCover(doc,payload,survey,client);doc.addPage();let y=reportsPdfHeader(doc,payload.title,payload.subtitle);
