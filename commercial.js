...
function commercialOpportunityCard(o){
  ...
  return `<article class="comm-card ${overdue?'is-overdue':''}" onclick="commercialOpenOpportunity('${o.id}')" tabindex="0" onkeydown="if(event.key==='Enter')commercialOpenOpportunity('${o.id}')">
    ...
    <div class="comm-card-bottom"><span>${esc(commSellerName(o.seller_id))}${o.indicator_id?` · ${esc(commIndicatorName(o.indicator_id))}`:''}</span><div class="comm-card-actions">${conversationButton(o.phone,'Olá '+(o.client_name||o.company||'')+'! Podemos conversar sobre sua pesquisa no PesquisaPro?')}<select aria-label="Mover oportunidade de estágio" onclick="event.stopPropagation()" onchange="commercialMoveStage('${o.id}',this.value)">${COMM_STAGES.map(s=>`<option value="${s.key}" ${o.stage===s.key?'selected':''}>${s.label}</option>`).join('')}</select></div></div>
  </article>`;
}
function commercialDetail(){
  ...
  return `<div class="comm-detail-actions"><button class="btn btn-out" onclick="commercialBackToBoard()">← Voltar ao funil</button><div><button class="btn btn-out" onclick="commercialEditOpportunity('${o.id}')">Editar</button>${canProposal?` <button class="btn btn-fill" onclick="commercialStartProposal('${o.id}')">＋ Criar proposta</button>`:''}</div></div>
    <div class="comm-detail-grid"><div class="card comm-detail-main">
      <div class="comm-detail-title"><div><div class="eyebrow">OPORTUNIDADE COMERCIAL</div><h2>${esc(o.company||o.client_name||'Sem empresa')}</h2><p>${esc(o.client_name||'Contato não informado')} ${o.city?'· '+esc(o.city):''}</p></div><div class="comm-detail-title-actions">${conversationButton(o.phone,'Olá '+(o.client_name||o.company||'')+'! Podemos conversar sobre sua proposta de pesquisa?')}<span class="comm-stage-badge" style="--stage-color:${COMM_STAGES.find(s=>s.key===o.stage)?.color||'#64748b'}">${esc(COMM_STAGE_LABEL[o.stage]||o.stage)}</span></div></div>
