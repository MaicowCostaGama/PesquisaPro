...
PAGES.commercial=()=>{
  if(!['admin','admpro','vendedor','coord','gerente'].includes(selectedRole)){
...
function commercialPaint(){
  ...
  root.innerHTML=COMM_VIEW==='form'?commercialOpportunityForm():COMM_VIEW==='detail'?commercialDetail():commercialBoard();
}
function commercialBoard(){
  ...
  <div class="comm-board-note">Vendedores visualizam apenas suas oportunidades. Administradores e perfis de gestão podem acompanhar o funil inteiro e atribuir o responsável.</div>`;
}
function commercialOpportunityCard(o){
  ...
  return `<article class="comm-card ${overdue?'is-overdue':''}" onclick="commercialOpenOpportunity('${o.id}')" tabindex="0" onkeydown="if(event.key==='Enter')commercialOpenOpportunity('${o.id}')">
    ...
    <div class="comm-card-bottom"><span>${esc(commSellerName(o.seller_id))}</span><select aria-label="Mover oportunidade de estágio" onclick="event.stopPropagation()" onchange="commercialMoveStage('${o.id}',this.value)">${COMM_STAGES.map(s=>`<option value="${s.key}" ${o.stage===s.key?'selected':''}>${s.label}</option>`).join('')}</select></div>
  </article>`;
}
function commercialDetail(){
  ...
  return `<div class="comm-detail-actions"><button class="btn btn-out" onclick="commercialBackToBoard()">← Voltar ao funil</button><div><button class="btn btn-out" onclick="commercialEditOpportunity('${o.id}')">Editar</button>${commStaff()?` <button class="btn btn-fill" onclick="commercialStartProposal('${o.id}')">＋ Criar proposta</button>`:` <button class="btn btn-fill" onclick="commercialStartProposal('${o.id}')">＋ Criar proposta</button>`}</div></div>
    <div class="comm-detail-grid"><div class="card comm-detail-main">
      ...
      <div class="comm-stage-edit"><label class="lbl">Atualizar estágio</label><select class="inp" onchange="commercialMoveStage('${o.id}',this.value)">${COMM_STAGES.map(s=>`<option value="${s.key}" ${o.stage===s.key?'selected':''}>${s.label}</option>`).join('')}</select></div>
...
function commercialOpportunityForm(){
  const isNew=!COMM_EDIT_ID;const o=isNew?{client_name:'',company:'',email:'',phone:'',city:'',source:'Indicação',survey_type:'',estimated_interviews:'',region:'',expected_value:'',next_action_at:'',description:'',notes:'',seller_id:selectedRole==='vendedor'?CURRENT_PROFILE.id:''}:COMM_OPPORTUNITIES.find(x=>x.id===COMM_EDIT_ID)||{};
